...
class sbmtm():
    ...
    def make_graph_from_adj(self, adj):
        ...
        an adjacency matrix.
        1st column contains words
        Remaining columns contain p(word|topic),
        column names are the topic names
        '''
        # Find number of words and topics
        n_words = adj.shape[0]
        n_topics = adj.shape[1]
...
#         Use type double as edges are probabilites
#         keeping the name as "count" incase that 
#         is needed for anything.
        ecount = g.ep["count"] = g.new_ep("double")

        # Loop through topics (skip 1st column as it's not a topic)
        for itopic in range(1,n_topics):
            # Add topic name from adj column name
            topic = adj.columns[itopic]
            d = docs_add[topic]
            name[d] = topic
            kind[d] = 0
            # Loop through words
            for iword in range(n_words):
                # Add word (from 1st column)
                word = adj.iloc[iword,0]
                w = words_add[word]
                name[w] = word
                kind[w] = 1
                # Add edegs for non-zero probs p(word | topic)
                if adj.iloc[iword,itopic] > 0:
                    e = g.add_edge(d,w)
                    # Weight(called count) is p(word | topic)
                    ecount[e] = adj.iloc[iword,itopic]
        self.g = g
